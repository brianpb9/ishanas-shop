...
function CustomerView({ customer, state, size }) {
  ...
  return (
    <div className="relative w-full flex items-end justify-center">
      ...
      {/* slight lean toward Ishana, and mirror so the customer turns to face her */}
      <div style={{ transform: "rotate(-4deg)", transformOrigin: "bottom center", width: "100%" }}>
        <div style={{ transform: "scaleX(-1)" }}>
          <ImgOr src={"assets/animal-" + customer.type + ".png"} className={anim + " object-contain"} style={{ ...imgStyle, filter: "drop-shadow(0 4px 6px rgba(0,0,0,0.15))" }} fallback={<div className={anim}><ChibiAnimal type={customer.type} state={freeze ? "freeze" : "wait"} size={size || 150} /></div>} />
        </div>
      </div>
    </div>
  );
...
function ShopScene({ tierIdx, world, shop, placed, equipped, pet, ishanaMood = "idle", customer, customerHappy, custKey, freeze, mode = "serve", preview, editable, height = 300, onItemDown, onMove, onUp, sceneRef, children }) {
  ...
  return (
    <div ref={sceneRef} onPointerMove={editable ? onMove : undefined} onPointerUp={editable ? onUp : undefined} onPointerLeave={editable ? onUp : undefined}
      ...
      {mac && <div className={"absolute " + pulse("machine")} style={{ left: "16%", bottom: "33%", width: "17%", zIndex: 2, filter: fancy(mac.rarity) ? "drop-shadow(0 0 10px gold)" : "drop-shadow(0 3px 5px rgba(0,0,0,.15))" }}>
      ...
      {helper && <div className={"absolute " + (customerHappy ? "cust-happy" : "bob") + " " + pulse("helper")} style={{ left: "0%", bottom: "33%", width: "16%", zIndex: 2, filter: "drop-shadow(0 3px 5px rgba(0,0,0,.2))" }} title="Your staff">
      ...
      {pet && ITEM[pet] && (
        <div className="absolute pet-hop flex items-end justify-center" style={{ left: "4%", bottom: "3%", zIndex: 6 }} title="Your pet">
          <div className="relative leading-none" style={{ fontSize: 42, filter: "drop-shadow(0 4px 4px rgba(0,0,0,.22))" }}>{ITEM[pet].emoji}<div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: 0, width: 30, height: 7, background: "rgba(0,0,0,.12)", borderRadius: "50%" }} /></div>
        </div>
      )}

      {/* ---------- CUSTOMER — serve mode only; slides in, faces Ishana ---------- */}
      {mode === "serve" && customer && (
        <div key={custKey} className="absolute cust-enter" style={{ right: "0%", bottom: "4%", width: "48%", zIndex: 7 }}>
