...
const CATALOG = [
  ...
  it("giantunicorn", "decor", "Giant Unicorn Statue", "dream", { emoji: "🦄", cost: 100000 }),
  it("rainbowcastle", "decor", "Rainbow Castle Shop", "dream", { emoji: "🏰", cost: 250000 }),
  it("palace", "decor", "Ice Cream Palace", "dream", { emoji: "🍨", cost: 500000 }),
...
function CustomerView({ customer, state }) {
  return <div className="relative flex flex-col items-center">{customer.golden && <div className="absolute -inset-3 golden-glow" />}<div className="relative"><ChibiAnimal type={customer.type} state={state} /></div></div>;
}
...
function ShopScene({ tierIdx, world, shop, placed, editable, height = 300, onItemDown, onMove, onUp, sceneRef, children }) {
  ...
  return (
    <div ref={sceneRef} onPointerMove={editable ? onMove : undefined} onPointerUp={editable ? onUp : undefined} onPointerLeave={editable ? onUp : undefined}
      ...
      {!wall && <div className="absolute inset-0" style={{ backgroundImage: "repeating-linear-gradient(90deg, rgba(255,150,190,0.16) 0 18px, transparent 18px 36px)" }} />}
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: (wall && wall.dark) ? "radial-gradient(#ffffff66 1px,transparent 1px)" : "radial-gradient(#ffffff 2px, transparent 2px)", backgroundSize: "26px 26px" }} />
      {/* hanging pendant lamps */}
      <div className="absolute left-0 right-0 flex justify-around px-6" style={{ top: 0, zIndex: 1 }}>{[0, 1, 2].map((i) => (<div key={i} className="flex flex-col items-center"><div style={{ width: 2, height: 10, background: "#cfa9a9" }} /><div style={{ width: 18, height: 12, borderRadius: "0 0 50% 50%", background: "linear-gradient(#ff9ec4,#ff7eb3)", boxShadow: "0 5px 10px rgba(255,150,190,.45)" }} /></div>))}</div>
...
function StorefrontScene({ tierIdx, world, shop, placed, equipped, pet, ishanaMood, customer, freeze, height = 300, children }) {
  const I = INTERIORS[tierIdx];
  const S = (k) => (shop[k] ? ITEM[shop[k]] : null);
  ...
  return (
    <div className="relative rounded-[1.6rem] overflow-hidden shadow-2xl w-full" style={{ height, minHeight: 200, border: "4px solid #ffd1e3" }}>
      {/* sky + far scenery */}
      <div className="absolute inset-0" style={{ background: "linear-gradient(" + world.sky[0] + "," + world.sky[1] + ")" }} />
      <div className="absolute" style={{ top: 8, left: 10, fontSize: 22, opacity: 0.9 }}>{world.sun}</div>
      <div className="absolute float-slow" style={{ top: 10, left: "30%", fontSize: 20, opacity: 0.85 }}>☁️</div>
      <div className="absolute float-slow" style={{ top: 20, right: "26%", fontSize: 16, opacity: 0.8, animationDelay: "1.5s" }}>☁️</div>
      <div className="absolute float-slow" style={{ top: 14, right: 12, fontSize: 22, opacity: 0.85 }}>{world.out}</div>
      {/* sidewalk (outside ground) */}
      <div className="absolute bottom-0 left-0 right-0" style={{ height: "24%", background: "linear-gradient(#f2ead9,#ddd0bb)", borderTop: "3px solid rgba(0,0,0,0.08)", backgroundImage: "repeating-linear-gradient(90deg, rgba(0,0,0,0.05) 0 1px, transparent 1px 46px)" }} />
      {/* greenery + lamp post + sandwich board */}
      <div className="absolute" style={{ bottom: "20%", left: 0, fontSize: 26 }}>🌳</div>
      <div className="absolute" style={{ bottom: "20%", right: 0, fontSize: 26 }}>🌳</div>
      <div className="absolute" style={{ bottom: "3%", left: "3%", fontSize: 22 }}>🪴</div>
      <div className="absolute leading-tight text-center rounded-md px-1 py-0.5" style={{ bottom: "3%", right: "4%", fontSize: 9, fontWeight: 900, color: "#fff", background: "#5a3b2a" }}>Today's<br/>Special 🍨</div>
      ...
      <div className="absolute" style={{ left: "5%", right: "5%", top: "7%", height: "66%" }}>
        {roof ? (
          ...
          <div className="absolute left-[-3%] right-[-3%]" style={{ top: -12, height: 18, zIndex: 3, background: "repeating-linear-gradient(90deg,#ff8fb3 0 14px,#fff 14px 28px)", borderRadius: "10px 10px 0 0" }}>
            <div className="absolute left-0 right-0 flex justify-around" style={{ bottom: -7 }}>{Array.from({ length: 11 }).map((_, i) => <div key={i} style={{ width: 13, height: 13, borderRadius: 99, background: i % 2 ? "#fff" : "#ff8fb3" }} />)}</div>
          </div>
        )}
        {/* building body */}
        ...
        {/* OPEN sign */}
        <div className="absolute rounded-md px-1.5 py-0.5 text-[8px] font-black text-white" style={{ right: "8%", top: "26%", background: "#ff5c93", zIndex: 4, transform: "rotate(-4deg)" }}>OPEN</div>
        {/* sign */}
      ...
      {/* CUSTOMER — outside, in front of the serving counter */}
      <div className="absolute" style={{ left: "44%", bottom: "3%", zIndex: 6, transform: "scale(0.8)", transformOrigin: "bottom center" }}>
        <CustomerView customer={customer} state={freeze ? "freeze" : "wait"} />
      </div>
...
function CountingMode({ level, onSolve, onMiss, locked, sounds }) {
...
function SortingMode({ level, onSolve, onMiss, locked, sounds }) {
...
function MegaMode({ level, onSolve, locked, sounds }) {
...
function RareArrival({ customer, onContinue }) { return (<Overlay><Confetti show /><p className="text-lg font-bold" style={{ color: "#a06bff" }}>✨ NEW FRIEND! ✨</p><div className="my-2 reward-pop flex justify-center"><ChibiAnimal type={customer} state="wait" size={104} /></div><h2 className="text-2xl font-black mb-1" style={{ color: "#ff5c93" }}>{CUSTOMER_INFO[customer].name}</h2><p className="text-sm text-gray-500 mb-3">New sticker for your album! 📖</p><button onClick={onContinue} className="pulse-btn text-white text-xl font-black px-8 py-3 rounded-full shadow-xl" style={{ background: "linear-gradient(#a06bff,#7b3ff2)" }}>Cool! 🌟</button></Overlay>); }
...
function ItemGrid({ items, save, onUse, isEquipped, labelFor, noDisable }) {
  return (<div className="grid grid-cols-3 gap-2">{items.map((i) => {
    ...
    return (<button key={i.id} onClick={() => onUse(i)} disabled={!noDisable && !owned && !afford} className="relative rounded-2xl p-2 shadow-md transition active:scale-95 text-center flex flex-col items-center" style={{ background: equipped ? "#fff0d6" : owned ? "#f3fff3" : afford ? "#fff" : "#f3f3f3", border: "3px solid " + (equipped ? "#ffb300" : owned ? "#7ed957" : "#eee"), opacity: owned || afford ? 1 : 0.6 }}>
      ...
      <div className="text-[10px] font-black mt-0.5" style={{ color: equipped ? "#ff7e1d" : owned ? "#4caf2f" : "#ff7e1d" }}>{labelFor ? labelFor(i, owned, equipped) : (equipped ? "Using ✓" : owned ? "Use it" : "🪙 " + money(i.cost))}</div>
    </button>);
  })}</div>);
}
...
function DressUpPage({ save, onUse }) {
  ...
  return (<div>
    <div className="flex justify-center mb-2"><FullIshana equipped={previewEquipped} pet={previewPet} size={138} mood="idle" /></div>
    {tryOn && (
        ...
        {sub === "themes" ? (
          ...
          active.cats.map((c) => (
            <div key={c} className="mb-2">
              ...
              <ItemGrid items={byCat(c)} save={save} onUse={onUse} isEquipped={(i) => (isSlot(i.cat) ? save.shop[i.cat] === i.id : placedIds.includes(i.id))} labelFor={(i, o, e) => (isSlot(i.cat) ? (e ? "Using ✓" : o ? "Use it" : "🪙 " + money(i.cost)) : (placedIds.includes(i.id) ? "In shop ✓" : o ? "Place it" : "🪙 " + money(i.cost)))} />
            </div>
          ))
        )}
      </div>
...
function IshanaShop() {
  ...
  const onSolve = useCallback((opts = {}) => {
    ...
    const coinsGain = Math.round((14 + save.level * 3 + (opts.bonusCoins || 0) * 3) * mult * sb * themeBonus);
    ...
    after(120, () => setShake(false)); after(420, () => { sounds.gulp(); setShowNyam(true); }); after(620, () => { sounds.yay(); sounds.coin(); });
  ...
  return (
    <div className={"w-full flex justify-center " + (shake ? "screen-shake" : "")} style={{ height: "100dvh", overflow: "hidden", background: "linear-gradient(160deg,#fff0f6,#e7f9ff 55%,#fff7e6)", paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      ...
      <div className="w-full max-w-md mx-auto relative z-10 flex flex-col" style={{ height: "100%" }}>
        ...
        {screen === "welcome" && (
          ...
          <div className="text-center rounded-[2rem] p-6 shadow-2xl w-full overflow-y-auto" style={{ maxHeight: "100%", background: "rgba(255,255,255,0.78)", border: "4px solid #ffd1e3" }}>
            <h1 className="text-4xl font-black mb-1" style={{ color: "#ff5c93" }}>Ishana's Shop</h1>
            <p className="text-base mb-4 font-bold" style={{ color: "#8a5cff" }}>🍦 Build the coolest ice cream shop! 🍦</p>
        ...
        {screen === "play" && (
          <div className="flex-1 min-h-0 flex flex-col p-2">
            <div className="shrink-0 flex items-center justify-between gap-1 mb-2 text-xs sm:text-sm">
              ...
              {tab === "shop" && (
                <div className="h-full flex flex-col min-h-0">
                  ...
                  <div className="flex-1 min-h-0">
                    <StorefrontScene tierIdx={tierIdx} world={world} shop={save.shop} placed={save.placed} equipped={save.equipped} pet={save.pet} ishanaMood={feedback === "correct" ? "cheer" : brainFreeze ? "freeze" : "idle"} customer={customer} freeze={brainFreeze} height="100%">
                  ...
                  <div className={"shrink-0 " + (shake && feedback === "wrong" ? "shake" : "")}><ModeComp key={roundKey} level={save.level} locked={locked} sounds={sounds} onSolve={onSolve} onMiss={onMiss} /></div>
