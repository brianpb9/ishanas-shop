...
function CustomerView({ customer, state, size }) {
  ...
  return (
    <div className="relative w-full flex items-end justify-center">
      ...
      {!happy && !freeze && (
        <div className="order-bob absolute" style={{ top: "-2%", left: "-6%", zIndex: 8 }}>
          <div className="relative bg-white rounded-2xl px-2.5 py-1.5 shadow-md" style={{ border: "2px solid #ffd1e3", fontSize: 22, lineHeight: 1 }}>🍦
            <div className="absolute" style={{ left: 8, bottom: -8, width: 0, height: 0, borderLeft: "7px solid transparent", borderRight: "7px solid transparent", borderTop: "9px solid #fff" }} />
          </div>
        </div>
      )}
...
function ShopScene({ tierIdx, world, shop, placed, equipped, pet, ishanaMood = "idle", customer, customerHappy, custKey, freeze, mode = "serve", preview, editable, height = 300, onItemDown, onMove, onUp, sceneRef, children }) {
  ...
  return (
    <div ref={sceneRef} onPointerMove={editable ? onMove : undefined} onPointerUp={editable ? onUp : undefined} onPointerLeave={editable ? onUp : undefined}
      ...
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: (wall && wall.dark) ? "radial-gradient(#ffffff66 1px,transparent 1px)" : "radial-gradient(#ffffff 2px, transparent 2px)", backgroundSize: "30px 30px" }} />
      ...
      {/* ---------- BACK COUNTER ledge — a workstation surface the machine, staff
           & helper visibly stand ON (so they don't float against the wall) ---------- */}
      <div className="absolute left-0 right-0" style={{ bottom: "31%", height: "9%", background: "linear-gradient(#fff5ec,#ffe3d0)", borderTop: "3px solid #ffffff", boxShadow: "0 -3px 7px rgba(0,0,0,0.07)", zIndex: 1 }} />
      ...
      {mac && <div className={"absolute " + pulse("machine")} style={{ left: "12%", bottom: "35%", width: "16%", zIndex: 2, filter: fancy(mac.rarity) ? "drop-shadow(0 0 10px gold)" : "drop-shadow(0 3px 5px rgba(0,0,0,.15))" }}>
      ...
      {helper && <div className={"absolute " + (customerHappy ? "cust-happy" : "bob") + " " + pulse("helper")} style={{ left: "1%", bottom: "35%", width: "14%", zIndex: 2, filter: "drop-shadow(0 3px 5px rgba(0,0,0,.2))" }} title="Your staff">
      ...
      {placed.map((p) => { const D = ITEM[p.id]; if (!D) return null; const zn = D.zone || "floor"; const front = zn === "floor" || zn === "counter"; const art = DECOR_ART[p.id]; const w = zn === "wall" ? "15%" : zn === "shelf" ? "12%" : "17%"; return (
        <div key={p.id} onPointerDown={editable ? (e) => onItemDown(e, p.id) : undefined} onDoubleClick={editable ? () => onUp(p.id, true) : undefined}
          ...
          {art ? (
            ...
            <div className="relative leading-none" style={{ fontSize: front ? 29 : 26, filter: "drop-shadow(0 4px 3px rgba(0,0,0,.18))" }}>{D.emoji}{front && <div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: -4, width: 24, height: 6, background: "rgba(0,0,0,.12)", borderRadius: "50%" }} />}</div>
          )}
        </div>
      ); })}
...
function SortingMode({ level, onSolve, onMiss, locked, sounds, speak }) {
  ...
  return (<><Bubble><p className="text-base font-bold" style={{ color: "#6b4bff" }}>Catch <span style={{ color: f.color }}>{need} {f.name}</span> scoops! 🌧️</p><p className="text-sm text-gray-500">Tap only the right colour — {got}/{need}</p></Bubble><div className="relative rounded-2xl overflow-hidden mb-1" style={{ height: 200, background: "linear-gradient(#eef9ff,#dff1ff)", border: "3px solid #b3e5ff" }}><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center" style={{ opacity: 0.3 }}><div className="text-3xl">☁️🍦☁️</div><span className="text-xs font-black text-sky-500 mt-1">Tap the falling {f.name} scoops!</span></div>{items.map((x) => <button key={x.id} onClick={() => grab(x)} className="absolute falling active:scale-90" style={{ left: x.left + "%", top: -50, animationDuration: x.dur + "s" }}><Scoop flavorKey={x.color} size={46} /></button>)}</div></>);
}
...
function MegaMode({ level, onSolve, locked, sounds, speak }) {
  ...
  return (<><Bubble><p className="text-base font-bold" style={{ color: "#6b4bff" }}>🗼 MEGA ORDER! Build a <span className="text-pink-500">{target}-scoop</span> tower!</p><p className="text-sm text-gray-500">{tower.length} / {target}</p></Bubble><div className="relative rounded-2xl overflow-y-auto mb-2 flex items-end justify-center" style={{ height: 170, background: "linear-gradient(#fff,#fff5fa)", border: "3px solid #ffd1e3" }}>{tower.length === 0 && <div className="pointer-events-none absolute inset-0 flex items-center justify-center text-center px-4 font-black" style={{ color: "#ffc2dd", fontSize: 15 }}>Tap “+ Add Scoop” to stack the tower! 🍦</div>}<div className="scale-90"><Cone scoops={tower} size={26} max={30} /></div></div><button onClick={add} disabled={locked} className="w-full rounded-2xl py-5 text-2xl font-black text-white shadow-md active:scale-95 transition" style={{ background: "linear-gradient(#ff9f43,#ff7e1d)" }}>+ Add Scoop 🍦</button></>);
}
