...
function CustomerView({ customer, state, size }) {
  const cls = state === "freeze" ? "shiver" : "bob";
  ...
  return <div className="relative w-full flex items-end justify-center">{customer.golden && <div className="absolute -inset-3 golden-glow" />}
    <ImgOr src={"assets/animal-" + customer.type + ".png"} className={cls + " object-contain"} style={{ ...imgStyle, filter: "drop-shadow(0 4px 6px rgba(0,0,0,0.15))" }} fallback={<div className={cls}><ChibiAnimal type={customer.type} state={state} size={size || 150} /></div>} />
  </div>;
}
...
function ShopScene({ tierIdx, world, shop, placed, equipped, pet, ishanaMood = "idle", customer, freeze, mode = "serve", preview, editable, height = 300, onItemDown, onMove, onUp, sceneRef, children }) {
  ...
  return (
    <div ref={sceneRef} onPointerMove={editable ? onMove : undefined} onPointerUp={editable ? onUp : undefined} onPointerLeave={editable ? onUp : undefined}
      ...
      <div className={"absolute left-0 right-0 bottom-0 " + pulse("floor")} style={{ height: "38%", background: floor ? floor.bg : "linear-gradient(" + I.floor + "," + I.floor + "dd)", borderTop: "3px solid rgba(255,255,255,0.5)", zIndex: 1 }} />
      ...
      <div className="absolute" style={{ right: "4%", top: roof ? "14%" : "9%", width: "30%", zIndex: 1, filter: "drop-shadow(0 3px 5px rgba(0,0,0,.08))" }}>
      ...
      {/* ---------- helper + machine flanking Ishana (on cute pedestals) ---------- */}
      {helper && <div className={"absolute bob flex items-center justify-center rounded-2xl " + pulse("helper")} style={{ left: "5%", bottom: "19%", width: "16%", aspectRatio: "1", fontSize: 30, background: "radial-gradient(circle at 50% 40%, #ffffffcc, #ffe6f1cc)", boxShadow: "0 4px 8px rgba(0,0,0,.1)", zIndex: 4 }}>{helper.emoji}</div>}
      {mac && <div className={"absolute " + pulse("machine")} style={{ right: "5%", bottom: "16%", width: "21%", zIndex: 4, filter: fancy(mac.rarity) ? "drop-shadow(0 0 10px gold)" : "drop-shadow(0 3px 5px rgba(0,0,0,.15))" }}>
        <ImgOr src={ASSET.machine[mac.base]} className="w-full object-contain" fallback={<div className="flex items-center justify-center rounded-2xl" style={{ aspectRatio: "1", fontSize: 30, background: "radial-gradient(circle at 50% 40%, #ffffffcc, #e6f7ffcc)", boxShadow: "0 4px 8px rgba(0,0,0,.1)" }}>{mac.emoji}</div>} />
      </div>}
      ...
      {placed.map((p) => ITEM[p.id] && (
        <div key={p.id} onPointerDown={editable ? (e) => onItemDown(e, p.id) : undefined} onDoubleClick={editable ? () => onUp(p.id, true) : undefined}
          className={"absolute flex items-end justify-center " + (editable ? "cursor-grab active:cursor-grabbing" : "")} style={{ left: p.x + "%", top: p.y + "%", transform: "translate(-50%,-50%)", zIndex: 3, touchAction: "none" }} title={editable ? "Drag · double-tap to put away" : undefined}>
          <div className="relative leading-none" style={{ fontSize: 34, filter: "drop-shadow(0 4px 3px rgba(0,0,0,.18))" }}>{ITEM[p.id].emoji}<div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: -4, width: 26, height: 7, background: "rgba(0,0,0,.12)", borderRadius: "50%" }} /></div>
        </div>
      ))}
      ...
      {/* ---------- CUSTOMER — serve mode only, big enough to feel real ---------- */}
      {mode === "serve" && customer && (
        <div className="absolute" style={{ right: "0%", bottom: "4%", width: "48%", zIndex: 6 }}>
...
function DecoratePage({ save, setSave, sounds, world, tierIdx }) {
  ...
  const buyFocused = () => {
    ...
    setSave((s) => {
      ...
      else ns.placed = [...s.placed, { id: i.id, x: 30 + Math.random() * 40, y: 46 + Math.random() * 22 }];
      return ns;
    });
    setFocus(null);
  };
...
function IshanaShop() {
  ...
  const onSolve = useCallback((opts = {}) => {
    ...
    const coinsGain = Math.round((10 + save.level * 2 + (opts.bonusCoins || 0) * 3) * mult * sb * themeBonus);
